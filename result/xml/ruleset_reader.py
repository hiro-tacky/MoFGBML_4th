...
class RuleSetXML(XML):
    ...
    def fuzzyTermslabel(self, categorizeType, initializationValue = -1, gen = gen_plot, includeDontCare = False):
        FuzzyTermslabel = [[{} for i in range(self.attributeNum)] for j in range(trial_num)]#0番目試行のKを使用
        if categorizeType == "partitionNum":
            for trialKey, trial in self.ruleset.items():                              
                kb = trial[gen].kb
                for dim, fuzzySets_dim in kb.fuzzySets.items():
                    for ID, fuzzyTerm in fuzzySets_dim.items():
                        if not includeDontCare and ID == 0:
                            continue
                        name = fuzzyTerm.name.split('_')[0]
                        partitionNum = fuzzyTerm.PartitionNum
                        if name in FuzzyTermslabel[trialKey][dim]:
                            if not partitionNum in FuzzyTermslabel[trialKey][dim][name]:
                                FuzzyTermslabel[trialKey][dim][name][partitionNum] = initializationValue
                        else:
                            FuzzyTermslabel[trialKey][dim][name] = {partitionNum : initializationValue}
        elif categorizeType == "name":
            for trialKey, trial in self.ruleset.items():                              
                kb = trial[gen].kb
                for dim, fuzzySets_dim in kb.fuzzySets.items():
                    for ID, fuzzyTerm in fuzzySets_dim.items():
                        if not includeDontCare and ID == 0:
                            continue
                        name = fuzzyTerm.name.split('_')[0]
                        if not name in FuzzyTermslabel[trialKey][dim]:
                            FuzzyTermslabel[trialKey][dim][name] = initializationValue
        return FuzzyTermslabel

    def UsedMenbershipDataRank(self, gen = gen_plot, includeDontCare = False, isCoverAllClasses = False):
